...
function createPrismaClient() {
  ...
  if (databaseUrl && (databaseUrl.startsWith('postgres://') || databaseUrl.startsWith('postgresql://'))) {
    try {
      ...
      console.log('PostgreSQL configured with optimized serverless settings')
    } catch (error) {
...
export async function executePrismaOperation<T>(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      if (attempt > 0) {
        console.log(`Prisma operation retry ${attempt + 1}/${maxRetries}`)
      }
      ...
      lastError = error
      console.error(`Prisma operation attempt ${attempt + 1} failed:`, error?.message || error)
