...
const RedTeamAgent_1 = require("../services/RedTeamAgent");
 ...
 */
router.post('/start', validation_1.validateAssessmentRequest, async (req, res) => {
    try {
        const { targetName, targetDescription, chatAgentUrl, chatAgentConfig, openrouterApiKey, selectedModel } = req.body;
        const assessmentId = (0, uuid_1.v4)();
        const io = req.app.get('io');
        // Validate chat agent URL
        if (!ChatAgentConnector_1.ChatAgentConnector.validateUrl(chatAgentUrl)) {
        ...
        const chatConnector = new ChatAgentConnector_1.ChatAgentConnector(defaultConfig);
        ...
        const redTeamAgent = new RedTeamAgent_1.RedTeamAgent(openrouterApiKey, selectedModel);
...
router.get('/:id/report', async (req, res) => {
    ...
    if (assessment && assessment.results?.vulnerabilityReport) {
        ...
        try {
            const dbAssessment = await prisma_1.prisma.assessment.findUnique({
                where: { id },
                include: {
                    findings: true,
                    exploitResults: true
            ...
            if (dbAssessment.vulnerabilityReport) {
                ...
                const mockAgent = new RedTeamAgent_1.RedTeamAgent('dummy-key');
...
router.post('/detect-config', async (req, res) => {
    try {
        ...
        const detectedConfig = await ChatAgentConnector_1.ChatAgentConnector.autoDetectFormat(chatAgentUrl, apiKey);
        if (detectedConfig) {
            res.json({
                success: true,
 ...
 */
async function runAssessmentBackground(assessmentId, io) {
    const assessment = activeAssessments.get(assessmentId);
    if (!assessment)
        return;
    try {
        console.log(`🔍 Starting assessment ${assessmentId}`);
        const results = await assessment.agent.runSecurityAssessment(assessment.connector, undefined, // target name already set
...
function generateHtmlReport(report) {
    ...
    const vulnerableFindings = report.detailedFindings
        .flatMap((df) => df.findings.filter((f) => f.analysis.vulnerable));
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="section">
        ...
        <ul>
            ${report.systemAnalysis.system_constraints.map((constraint) => `<li>${constraint}</li>`).join('')}
        </ul>
        ` : ''}
        ...
        <ul>
            ${report.systemAnalysis.content_policies.map((policy) => `<li>${policy}</li>`).join('')}
        </ul>
        ` : ''}
    </div>
...
${report.systemAnalysis.system_constraints.length > 0 ? `
System Constraints:
${report.systemAnalysis.system_constraints.map((constraint, index) => `- ${constraint}`).join('\n')}
` : ''}

${report.systemAnalysis.content_policies.length > 0 ? `
Content Policies:
${report.systemAnalysis.content_policies.map((policy, index) => `- ${policy}`).join('\n')}
` : ''}
...
${vectorFindings.findings.filter((f) => f.analysis.vulnerable).map((finding, index) => `
