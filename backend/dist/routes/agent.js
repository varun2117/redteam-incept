...
router.post('/test', async (req, res) => {
    try {
        ...
        if (!ChatAgentConnector_1.ChatAgentConnector.validateUrl(url)) {
        ...
        const connector = new ChatAgentConnector_1.ChatAgentConnector(connectionConfig);
...
router.post('/detect', async (req, res) => {
    try {
        ...
        const detectedConfig = await ChatAgentConnector_1.ChatAgentConnector.autoDetectFormat(url, apiKey);
        if (detectedConfig) {
            // Test the detected configuration
            const connector = new ChatAgentConnector_1.ChatAgentConnector(detectedConfig);
            const testResult = await connector.testConnection();
            res.json({
                success: true,
...
router.post('/validate-config', (req, res) => {
    try {
        ...
        else if (!ChatAgentConnector_1.ChatAgentConnector.validateUrl(config.url)) {
